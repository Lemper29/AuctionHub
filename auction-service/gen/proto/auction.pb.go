...
type Lot struct {
	...
	Name          string                 `protobuf:"bytes,2,opt,name=name,proto3" json:"name,omitempty"`                                        // Название лота
	Description   string                 `protobuf:"bytes,3,opt,name=description,proto3" json:"description,omitempty"`                          // Описание
	StartPrice    float64                `protobuf:"fixed64,4,opt,name=start_price,json=startPrice,proto3" json:"start_price,omitempty"`        // Начальная цена
	CurrentPrice  float64                `protobuf:"fixed64,5,opt,name=current_price,json=currentPrice,proto3" json:"current_price,omitempty"`  // Текущая цена
	CurrentWinner string                 `protobuf:"bytes,6,opt,name=current_winner,json=currentWinner,proto3" json:"current_winner,omitempty"` // ID текущего победителя
	Status        string                 `protobuf:"bytes,7,opt,name=status,proto3" json:"status,omitempty"`                                    // Статус: ACTIVE, COMPLETED, CANCELLED
	EndTimeUnix   int64                  `protobuf:"varint,8,opt,name=end_time_unix,json=endTimeUnix,proto3" json:"end_time_unix,omitempty"`    // Время окончания аукциона (Unix timestamp)
...
type Bid struct {
	...
	UserId        string                 `protobuf:"bytes,2,opt,name=user_id,json=userId,proto3" json:"user_id,omitempty"` // Идентификатор пользователя, сделавшего ставку
...
type CreateLotRequest struct {
	...
	DurationMinute int64                  `protobuf:"varint,4,opt,name=duration_minute,json=durationMinute,proto3" json:"duration_minute,omitempty"` // Длительность аукциона в секундах
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}
...
type PlaceBidResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Success       bool                   `protobuf:"varint,1,opt,name=success,proto3" json:"success,omitempty"`                        // Успешна ли ставка
	Message       string                 `protobuf:"bytes,2,opt,name=message,proto3" json:"message,omitempty"`                         // Сообщение (e.g., "Ставка принята", "Ставка слишком мала")
	UpdatedLot    *Lot                   `protobuf:"bytes,3,opt,name=updated_lot,json=updatedLot,proto3" json:"updated_lot,omitempty"` // Актуальное состояние лота
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}
...
const file_proto_auction_proto_rawDesc = "" +
	...
	"\rend_time_unix\x18\b \x01(\x03R\vendTimeUnix\"k\n" +
	"\x03Bid\x12\x15\n" +
	...
	"\x03lot\x18\x01 \x01(\v2\f.auction.LotR\x03lot2\xa5\x02\n" +
	"\x0eAuctionService\x12B\n" +
	"\tCreateLot\x12\x19.auction.CreateLotRequest\x1a\x1a.auction.CreateLotResponse\x129\n" +
	"\x06GetLot\x12\x16.auction.GetLotRequest\x1a\x17.auction.GetLotResponse\x12?\n" +
	"\bPlaceBid\x12\x18.auction.PlaceBidRequest\x1a\x19.auction.PlaceBidResponse\x12S\n" +
...
var file_proto_auction_proto_goTypes = []any{
	...
	(*GetLotRequest)(nil),          // 4: auction.GetLotRequest
	(*GetLotResponse)(nil),         // 5: auction.GetLotResponse
...
var file_proto_auction_proto_depIdxs = []int32{
	...
	2, // 4: auction.AuctionService.CreateLot:input_type -> auction.CreateLotRequest
	4, // 5: auction.AuctionService.GetLot:input_type -> auction.GetLotRequest
	6, // 6: auction.AuctionService.PlaceBid:input_type -> auction.PlaceBidRequest
	8, // 7: auction.AuctionService.SubscribeToLot:input_type -> auction.SubscribeToLotRequest
	3, // 8: auction.AuctionService.CreateLot:output_type -> auction.CreateLotResponse
	5, // 9: auction.AuctionService.GetLot:output_type -> auction.GetLotResponse
	7, // 10: auction.AuctionService.PlaceBid:output_type -> auction.PlaceBidResponse
	9, // 11: auction.AuctionService.SubscribeToLot:output_type -> auction.SubscribeToLotResponse
